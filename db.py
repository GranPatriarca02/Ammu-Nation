...
if not DB_URL or not API_TOKEN:
    raise Exception("ERROR: Debes revisar: DB_URL o API_TOKEN")

# Nombre de nuestra base de datos: 
DATABASE_NAME = "ammu-nation.db"

# _____ CONEXIÓN DE TURSO: _____
conn_turso = libsql.connect(DATABASE_NAME, sync_url=DB_URL, auth_token=API_TOKEN)
try:
    conn_turso.sync()
except Exception as e:
    print(f"ERROR: No se ha sincronizado con Turso. Se usara solo la base de datos local: {e}")

cursor_turso = conn_turso.cursor()

# _____ CONEXIÓN DE LOCAL (sqlite3): _____
conn_local = sqlite3.connect(DATABASE_NAME)
...
# Guarda los cambios en la base de datos local y en Turso, los sincronizamos.
def actualizarCommit():
    try:
        conn_turso.commit()
        conn_turso.sync() 
    except Exception as e:
        print("ERROR: Ha habido un problema con Turso: ", e)

# Cerramos las conexiones y cursores abiertos.
...
def crearTablas(cursor):
    try:
        ...
        print("INFO: Se han creado las tablas.")
    
    except Exception as e:
        print("ERROR: No se han podido crear las tablas", e)

# Obtenemos el ID de la tabla mediante el nombre
...
# Generamos un codigo de serie aleatorio y añadimos un prefijo según la categoria.
...
def ingesta_datos_auxiliares():
    ...
    # Recorremos los datos auxiliares para luego insertarlos en cada una de las tablas: 
...
def generar_productos_iniciales():
    ...
    # Creamos una lista vacia para almacenar los productos
    productos = []
    ...
    # MUNICIÓN (+30 PRODUCTOS, +10 POR CALIBRE COMUN)
...
def ingesta_productos_inicial():
    ...
    for p in productos_ejemplo:
        try:
            ...
            print(f"Error al insertar producto {p[1]}: {e}")

    actualizarCommit()
    print(f"Ingesta inicial de {contador_insertados} productos completada.")

# Verificamos si la tabla productos tiene algun registro.
def verificar_datos_existentes():
    # Usamos el cursor principal (Turso)
    cursor.execute("SELECT COUNT(*) FROM PRODUCTOS")
    return cursor.fetchone()[0] > 0

# En caso de que no tengamos las tablas creadas, ni los productos insertados: 
def inicializar_base_datos():

    print("___ INICIALIZANDO BASE DE DATOS ___")
    ...
    if not verificar_datos_existentes():
        print("INFO: La base de datos se encuentra vacia: Realizando ingesta inicial de datos.")
        ingesta_datos_auxiliares()
        ingesta_productos_inicial()
    else:
        print("ERROR: La base de datos ya contiene productos. Ingesta omitida.")
